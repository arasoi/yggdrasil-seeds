...
> A candidate list for expanding the catalog to cover Steam-distributed
> dedicated servers, tiered by how confident we are that each one runs on
> Linux without a Proton/Wine wrapper. Read on demand when picking the next
> seed to write.
...
(ARK: Survival Evolved), `2394010` (Palworld) and `896660` (Valheim) are
already built and left off this table.
